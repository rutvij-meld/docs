...
The bank picker widget emits Meld several events, including errors, that you should listen for in order to trigger certain actions on your end, for example closing the widget. These events can be viewed in the Console tab of your browser. While these events technically can be viewed by your user, their main purpose is to be used by Meld for debugging as well as for you to know when a connection has started and completed.
...
Once you have received the connect\_complete event, then congratulations, the widget flow is complete! After [setting up a webhook configuration with Meld](https://docs.meld.io/docs/webhook-quickstart#steps-to-receive-webhooks-from-meld), you can then expect the [BANK\_LINKING\_CONNECTION\_COMPLETED](https://docs.meld.io/docs/webhook-events-bank-linking#bank_linking_connection_completed) webhook to notify of official connection completion, followed by the other aggregation-related webhooks that relay when account/transaction data is ready to be fetched from the Meld API. 

See [Retrieving Connection Data](https://docs.meld.io/docs/retrieving-connection-data) for next steps and more detail regarding loading account & transaction data.
