...
These are the various webhooks you can expect to receive over the lifetime of a bank linking connection. To understand when to expect each webhook and what to do when it arrives, see [here](https://docs.meld.io/docs/when-is-your-data-available).
...
Sent once aggregation has completed for the accounts belonging to the connection. Products are now available to be fetched for the accounts. This webhook should arrive shortly after the `BANK_LINKING_ACCOUNTS_UPDATING` webhook, but sometimes products may take a while to aggregate for certain institutions. Note that if no products have been updated, then the financialAccounts object in this webhook will be empty. See more about [aggregation](https://docs.meld.io/docs/managing-completed-connections#account-aggregations) here.
...
Captures net transactional effects for each financial account belonging to the connection. Will be sent after each time an aggregation occurs in which transactions are added or modified. See the [webhook flow](https://docs.meld.io/docs/managing-completed-connections#webhook-flow) for more details.
