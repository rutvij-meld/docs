...
* Meld (and you) will still need to know which application is responsible for each transaction. When you call the [/widget endpoint](https://docs.meld.io/reference/crypto-session-widget-create), use the `externalSubAccountCustomerId` to pass in an identifier corresponding to the application that made the call. The `externalSubAccountCustomerId` you pass in will be tied to the transaction the user ends up making. This is useful for compliance, metrics, and revenue calculation.
...
The `externalSubAccountCustomerId` will be returned in the transaction webhooks and you can also query the [/transactions endpoint](https://docs.meld.io/reference/payments-transactions-search) with this value. This allows you to see all transactions made by a particular downstream application.
