...
> **Important:** Field locking only applies to the Meld Checkout interface. However, if you lock the sourceCurrencyCode/destinationCurrencyCode or walletAddress field in the Meld Checkout widget, then that field will also be locked in the partner's widget for the providers that support it. Read more about this [here](https://docs.meld.io/docs/white-label-cutomization#locking-fields).
