...
<Note>
To test this flow in Meld's sandbox, see [KYC testing](/docs/stablecoins/sandbox-guide/kyc-testing).
</Note>
...
You can keep track in your own system if a particular user has been KYCed via Meld. If you don't keep track, you can call `GET /accounts/customers`, passing the `customerId` of the user, to check whether the user has already been KYCed with Meld.
