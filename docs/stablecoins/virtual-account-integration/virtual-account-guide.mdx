...
API Endpoint: [POST /accounts/customers](https://docs.meld.io/reference/accounts-customers-create)
...
> **Note:** If you plan on using `DUENETWORK` later for executing transactions, then you must also specify the customer's address. This can be added to a customer by calling the `POST /accounts/customers/{customerId}/addresses` endpoint [here](https://docs.meld.io/reference/accounts-customers-addresses-create).
>
...
KYC will happen via Sumsub. If you have already KYC your users with Sumsub, you can pass in the KYC token, as described [here](https://docs.meld.io/update/docs/you-kyc-the-user). If instead you prefer Meld to KYC the user, that process is described in detail [here](https://docs.meld.io/update/docs/meld-kyces-the-user).

To test KYC in the sandbox, follow the steps [here](https://docs.meld.io/update/docs/kyc-testing).

**Initiate KYC:**

API Endpoints:

* [POST /accounts/customers/:customerId/kyc/initiate](https://docs.meld.io/reference/accounts-customers-kyc-initiate)
* [PATCH /accounts/customers/:customerId/kyc/initiate](https://docs.meld.io/reference/accounts-customers-kyc-update)
...
Alternatively, if you already have a user who has passed [KYC on your Sumsub account](https://docs.meld.io/docs/you-kyc-the-user), you can import it into Meld by sharing the Sumsub applicant token (TOKEN_IMPORT mode).

To import the KYC of the user, call the [initiate KYC endpoint](https://docs.meld.io/reference/accounts-customers-kyc-initiate). The customerId will be in the path, and your request body will look like this:
...
API Endpoint: [GET /accounts/customers/:customerId](https://docs.meld.io/reference/accounts-customers-create)
...
In both KYC initiation modes (**HOSTED_URL** and **TOKEN_IMPORT**), you can optionally specify Virtual Account providers you want to share the user's KYC with after Sumsub approves submission in the **kycShareProviders** field. Alternatively, you can add share providers later by calling the [Update KYC API](https://docs.meld.io/reference/accounts-customers-kyc-initiate).
...
* Noah
* Due Network. Requires customer country to be specified, see [Add an address to a customer](https://docs.meld.io/reference/accounts-customers-addresses-create)
...
After receiving a webhook event, you can call [ Search Customer API](https://docs.meld.io/reference/accounts-customers-search) for more details. For example, when the Virtual Account provider requires additional KYC on their platform, you will receive a webhook event with **kycRecipient.status=PENDING** status. Then, when calling [ Search Customer API](https://docs.meld.io/reference/accounts-customers-search), you will receive the following response
...
You will need to direct your user to `HostedURL` for them to pass additional KYC on the Virtual Account provider platform. After users complete additional KYC and the Virtual Account provider verifies that, you will receive another **CUSTOMER_KYC_STATUS_CHANGE** with **kycRecipient.status=APPROVED** status, and this will also be reflected in [Search Customer API](https://docs.meld.io/reference/accounts-customers-search) response.
...
You can find all the webhook events [here](https://docs.meld.io/docs/webhook-events). Verify webhook signatures per the [Webhooks Authentication](https://docs.meld.io/docs/webhooks-authentication) docs.

***

## 4. Get a Quote

API Endpoint: [POST /payments/virtual-account/quote](https://docs.meld.io/reference/crypto-virtual-account-quote-get)
...
API Endpoint: [POST /payments/virtual-account/onramp-order](https://docs.meld.io/reference/payments-virtual-account-ramp-onramp-order-create)
...
API Endpoint: [POST /payments/virtual-account/offramp-order](https://docs.meld.io/reference/payments-virtual-account-ramp-offramp-order-create)
...
Find more information on the various webhook event types [here](https://docs.meld.io/docs/webhook-events).
...
The `paymentTransactionStatus` field will display the current status of the transaction. Find the list of transaction statuses and their meanings [here](https://docs.meld.io/docs/transaction-statuses).

### Fetch the full transaction

Extract `paymentTransactionId` from the webhook and call:

API Endpoint: [GET /payments/transactions/:paymentTransactionId](https://docs.meld.io/reference/payments-transactions-get)
...
Provider sandboxes often cannot complete full settlement — you may only reach virtual bank account creation. See [Virtual Account Flow Testing Credentials](https://docs.meld.io/docs/copy-of-virtual-account-flow-testing-credentials) for provider-specific sandbox values (Noah, Due, Brale).
