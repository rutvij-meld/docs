...
Please refer to [Meld KYCes the User](https://docs.meld.io/update/docs/meld-kyces-the-user)
...
To import the KYC of the user, call the [initiate KYC endpoint](https://docs.meld.io/reference/accounts-customers-kyc-initiate). You will need to pass the Sumsub KYC token, generated for **meld.io_75082** client. The customerId will be in the path, and your request body will look like this:
...
If you completed previous steps and successfully KYCed the user with Meld or imported your Sumsub KYC applicant with Meld, all it takes is to share the KYC with the Network Partner **bypassKyc=true** in the body of the [Create Widget request](https://docs.meld.io/reference/crypto-session-widget-create). Meld then will recognize this and pass KYC data to Onramp through Sumsub. Onramp may still require some additional KYC in case Sumsub shared data is not enough, but all already provided data should be prefilled.
...
After receiving a webhook event, you can call [ Search Customer API](https://docs.meld.io/reference/accounts-customers-search) for more details. For example, when the Virtual Account provider requires additional KYC on their platform, you will receive a webhook event with **kycRecepient.status=PENDING** status. Then, when calling [ Search Customer API](https://docs.meld.io/reference/accounts-customers-search), you will receive the following response
